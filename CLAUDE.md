...
**Shell setup**: fish is the primary shell (`home/private_dot_config/fish/`), with `conf.d/*.fish` for tool integrations (loaded alphabetically, hence the `0-`/`00-` prefixes for load-order-sensitive files like PATH setup and Homebrew shellenv) and `functions/` for one-function-per-file fish functions. `dot_zshrc`/`dot_bashrc.tmpl`/`dot_zshenv.tmpl` exist as a minimal fallback/interop layer, not the primary shell config.
